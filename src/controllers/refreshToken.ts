...
export class Token {
  static async verifyAndRefreshToken(req: Request, res: Response, next: NextFunction) {
    ...
    jwt.verify(refresh_token, privateKey, async (err: any, user: {user_id: string}) => {
      ...
      const accessToken = generateAccessToken(parseInt(user.user_id), usr.user_type, privateKey);
